...
class PuckWorldEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.max_speed = 0.025  # max agent velocity along a axis 沿轴的最大代理速度

        self.re_pos_interval = 30  # 目标重置距离时间
        self.accel = 0.001  # agent 加速度
        self.rad = 0.05  # agent 半径,目标半径
        self.target_rad = 0.01  # target radius.
        self.goal_dis = 0.05  # 目标接近距离 expected goal distance
    ...
    def _step(self, action):
        ...
        pvx, pvy = pvx * 0.95, pvy * 0.95  # natural velocity loss 自然速度损失
        ...
        print("AGENT位置{},目标位置{},是否结束{}".format(dis,self.goal_dis,done))
